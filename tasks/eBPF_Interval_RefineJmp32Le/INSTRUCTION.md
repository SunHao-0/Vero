...
- **Optimality**: `γ(r1')` is contained in `γ(r')` for every `r'` that is also sound (and symmetrically for `r2'`)

This is the standard Galois connection optimality condition: `r1' = α({x ∈ γ(r1) | ∃y ∈ γ(r2), R(x,y)})`. If the relation is unsatisfiable (no feasible pair exists), the optimal result is ⊥ (empty gamma). Then prove correctness in the PROOF section. You may use the axiomatized tnum helpers (tnumStepUp/Down) from AXIOMS, and add helper lemmas in AUX.
