...
Implement the function in the IMPLEMENTATION section so that it computes the optimal tnum for the given binary operation on two tnums.

The function receives two tnums and must return a `Tnum` satisfying:
- **Well-formedness**: `(result.value &&& result.mask) = 0`
- **Soundness**: for every `x ∈ γ(a)` and `y ∈ γ(b)`, `op(x, y) ∈ γ(result)`
