# Task: Arithmetic Right Shift (32-bit)

**Source**: Linux eBPF verifier, kernel/bpf/verifier.c (adjust_scalar_min_max_vals, scalar32_min_max_*)
...
Implement the function in the IMPLEMENTATION section so that it computes the optimal abstract state for the given ALU operation.

The function receives two abstract register states (dst, src) and must return a `RegState` satisfying:
- **Soundness**: every concrete result `op(d, s)` for `d ∈ γ(dst)`, `s ∈ γ(src)` is in `γ(result)`
- **Optimality**: `result` is the best abstract transformer — the ⊑-least sound abstraction. For any other `RegState` `r'` that is also sound, `γ(result) ⊆ γ(r')`.

This is the standard Galois connection optimality condition: `result = α(f(γ(dst) × γ(src)))`. You may add helper lemmas in AUX.
