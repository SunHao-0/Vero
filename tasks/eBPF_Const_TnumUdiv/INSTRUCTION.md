# Task: Tnum Unsigned Division by Constant
...
Implement the function in the IMPLEMENTATION section so that it computes the optimal tnum for the given operation with a non-zero constant divisor.

The function receives a tnum and a non-zero `BitVec 64` constant, and must return a `Tnum` satisfying:
- **Well-formedness**: `(result.value &&& result.mask) = 0`
- **Soundness**: for every `x ∈ γ(tnum)`, `op(x, c) ∈ γ(result)`
- **Optimality**: `result` is the ⊑-least sound abstraction — for any other valid tnum `r'` that is also sound, `γ(result) ⊆ γ(r')`.

This is the standard Galois connection optimality condition: `result = α(f(γ(tnum)))`. You may add helper lemmas in AUX.
