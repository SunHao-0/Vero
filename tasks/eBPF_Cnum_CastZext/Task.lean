/-
  BPF cnum cast verification task: Zero Extension (cnum domain)
  Operator: ZEXT
  ...
  zero-extending the low size*8 bits to 64 bits (coerce_reg_to_size) on a single abstract register state in the
...
/-- Compute the optimal abstract state for Zero Extension (cnum domain). -/
