/-
  BPF cnum verification task: Register Bounds Synchronization
  Operator: REG_BOUNDS_SYNC
  Source: Linux eBPF verifier (bpf-next cnum domain), kernel/bpf/verifier.c (reg_bounds_sync)

  Compute the reduced canonical form of a register state in the
  bpf-next cnum + tnum domain: synchronize the tnum, the 64-bit cnum,
  and the 32-bit cnum against each other without changing the set of
  concrete values represented (the kernel's reg_bounds_sync()).
  The result must be sound and component-wise optimal: its cnums are
  minimum-size arc covers of γ(reg) and its low-32 projection, and its
  tnum mask is minimal. (A ⊑-least sound RegState does not always exist
  in the cnum domain — minimum-size arc covers can tie — hence the
...
/-- Compute the reduced canonical form of a register state. -/
...
theorem regBoundsSync_correct (reg : RegState)
    ...
    -- result's cnums are minimum-size arc covers of γ(reg) (and its
    -- low-32 projection) and its tnum mask is minimal
