# Task: Register Bounds Synchronization
...
Implement the function in the IMPLEMENTATION section so that it computes the reduced canonical form of a register state: synchronize the tnum, the 64-bit cnum, and the 32-bit cnum against each other (the kernel's `reg_bounds_sync()`).
...
The function receives a single abstract register state and must return a `RegState` satisfying:
