/-
  BPF cnum cast verification task: Sign Extension (cnum domain)
  Operator: SEXT
  ...
  sign-extending the low size*8 bits to 64 bits (coerce_reg_to_size_sx) on a single abstract register state in the
...
/-- Compute the optimal abstract state for Sign Extension (cnum domain). -/
