# Task: Sign Extension (cnum domain)
...
Implement the function in the IMPLEMENTATION section so that it computes the optimal abstract state for the given cast operation.
...
The function receives a single abstract register state and a `size` parameter (1, 2, or 4 bytes) and must return a `RegState` satisfying:
- **Soundness**: every concrete result `op(x, size)` for `x ∈ γ(reg)` is in `γ(result)`
...
Then prove correctness in the PROOF section. You may add helper lemmas in AUX.
