/-
  BPF ALU verification task: {{OPERATION_TITLE}}
  ...
  {{LEAN_OP_DESCRIPTION}} on two abstract register states.
...
def {{LEAN_CONCRETE_OP_NAME}} (d s : BitVec 64) : BitVec 64 :=
  {{LEAN_CONCRETE_OP_BODY}}

/-- Compute the optimal abstract state for {{OPERATION_TITLE}}. -/
def {{LEAN_FUNC_NAME}} (dst src : RegState) : RegState :=
-- === END: SPEC ===
...
theorem {{LEAN_FUNC_NAME}}_correct (dst src : RegState)
    (hv1 : validReg dst) (hv2 : validReg src)
    (hne1 : ∃ d, inGamma dst d) (hne2 : ∃ s, inGamma src s)
    {{LEAN_EXTRA_PRECONDITIONS}} :
    let r := {{LEAN_FUNC_NAME}} dst src
    -- Well-formedness: result is a valid register state
    validReg r ∧
    -- Soundness: every concrete result is in γ(r)
    (∀ d s, inGamma dst d → inGamma src s → inGamma r ({{LEAN_CONCRETE_OP_NAME}} d s)) ∧
