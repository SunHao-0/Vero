/-
  ...
  {{LEAN_OP_DESCRIPTION}} where the second operand is a non-zero constant.
...
def {{LEAN_CONCRETE_OP_NAME}} (a c : BitVec 64) : BitVec 64 :=
  {{LEAN_CONCRETE_OP_BODY}}

/-- Compute the optimal tnum for {{OPERATION_TITLE}}. -/
def {{LEAN_FUNC_NAME}} (a : Tnum) (c : BitVec 64) : Tnum :=
-- === END: SPEC ===
...
theorem {{LEAN_FUNC_NAME}}_correct (a : Tnum) (c : BitVec 64)
    (ha : validTnum a) (hc : c ≠ 0) :
    let r := {{LEAN_FUNC_NAME}} a c
    -- Well-formedness: result is a valid tnum
    validTnum r ∧
    -- Soundness: every concrete result satisfies r
    (∀ x, inTnumGamma a x →
      inTnumGamma r ({{LEAN_CONCRETE_OP_NAME}} x c)) ∧
