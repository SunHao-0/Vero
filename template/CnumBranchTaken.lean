/-
  BPF cnum JMP verification task: {{RELATION_TITLE}}
  Operator: {{OPERATOR_NAME}}
  Source: Linux eBPF verifier (bpf-next cnum domain), kernel/bpf/verifier.c (is_scalar_branch_taken)

  Determines if {{LEAN_REL_EXPR}} is always true, always false,
  or unknown for concrete values from two abstract register states in
  the bpf-next cnum + tnum domain: a 64-bit tnum plus 64-bit and 32-bit
...
/-- Decide branch outcome for {{RELATION_TITLE}}. -/
def {{LEAN_FUNC_NAME}} (reg1 reg2 : RegState) : BranchResult :=
-- === END: AXIOMS ===
...
def {{LEAN_ALWAYS_NAME}} (r1 r2 : RegState) : Prop :=
  ∀ (x y : BitVec 64), inGamma r1 x → inGamma r2 y → {{LEAN_REL_EXPR}}

/-- No pair (x, y) from gamma satisfies the relation. -/
def {{LEAN_NEVER_NAME}} (r1 r2 : RegState) : Prop :=
  ∀ (x y : BitVec 64), inGamma r1 x → inGamma r2 y → ¬({{LEAN_REL_EXPR}})

theorem {{LEAN_FUNC_NAME}}_correct (reg1 reg2 : RegState)
    (hv1 : validReg reg1) (hv2 : validReg reg2)
    (hne1 : ∃ x, inGamma reg1 x) (hne2 : ∃ y, inGamma reg2 y) :
    ({{LEAN_FUNC_NAME}} reg1 reg2 = BranchResult.always ↔ {{LEAN_ALWAYS_NAME}} reg1 reg2) ∧
    ({{LEAN_FUNC_NAME}} reg1 reg2 = BranchResult.never ↔ {{LEAN_NEVER_NAME}} reg1 reg2) ∧
    ({{LEAN_FUNC_NAME}} reg1 reg2 = BranchResult.always ∨
     {{LEAN_FUNC_NAME}} reg1 reg2 = BranchResult.never ∨
     {{LEAN_FUNC_NAME}} reg1 reg2 = BranchResult.unknown) := by
-- === END: SPEC ===
