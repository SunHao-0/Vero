/-
  BPF refinement verification task: {{RELATION_TITLE}}
  Operator: {{OPERATOR_NAME}}
  Source: Linux eBPF verifier, kernel/bpf/verifier.c (regs_refine_cond_op)

  Given two abstract register states, refine both to optimally contain
  only values that can participate in the relation {{LEAN_REL_EXPR}}.
...
/-- Refine register states on the branch-taken path for {{RELATION_TITLE}}. -/
def {{LEAN_FUNC_NAME}} (reg1 reg2 : RegState) : RegState × RegState :=
-- === END: AXIOMS ===
...
theorem {{LEAN_FUNC_NAME}}_correct (reg1 reg2 : RegState)
    ...
    (hfeas : ∃ x y, inGamma reg1 x ∧ inGamma reg2 y ∧ {{LEAN_REL_EXPR}}) :
    let (r1', r2') := {{LEAN_FUNC_NAME}} reg1 reg2
    ...
    (∀ x, inGamma reg1 x → (∃ y, inGamma reg2 y ∧ {{LEAN_REL_EXPR}}) → inGamma r1' x) ∧
    -- Soundness for r2': every feasible value is preserved
    (∀ y, inGamma reg2 y → (∃ x, inGamma reg1 x ∧ {{LEAN_REL_EXPR}}) → inGamma r2' y) ∧
    -- Optimality for r1': γ(r1') ⊆ γ(r') for all sound r'
    (∀ r', validReg r' → (∀ x, inGamma reg1 x → (∃ y, inGamma reg2 y ∧ {{LEAN_REL_EXPR}}) → inGamma r' x) →
            (∀ v, inGamma r1' v → inGamma r' v)) ∧
    -- Optimality for r2': γ(r2') ⊆ γ(r') for all sound r'
    (∀ r', validReg r' → (∀ y, inGamma reg2 y → (∃ x, inGamma reg1 x ∧ {{LEAN_REL_EXPR}}) → inGamma r' y) →
