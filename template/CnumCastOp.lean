/-
  BPF cnum cast verification task: {{OPERATION_TITLE}}
  Operator: {{OPERATOR_NAME}}
  Source: Linux eBPF verifier (bpf-next cnum domain), kernel/bpf/verifier.c (coerce_reg_to_size, coerce_reg_to_size_sx)

  Compute the optimal abstract state for the result of
  {{LEAN_OP_DESCRIPTION}} on a single abstract register state in the
  bpf-next cnum + tnum domain, parameterized by byte width.
...
def {{LEAN_CONCRETE_OP_NAME}} (x : BitVec 64) (size : Nat) : BitVec 64 :=
  {{LEAN_CONCRETE_OP_BODY}}

/-- Compute the optimal abstract state for {{OPERATION_TITLE}}. -/
def {{LEAN_FUNC_NAME}} (reg : RegState) (size : Nat) : RegState :=
-- === END: SPEC ===
...
theorem {{LEAN_FUNC_NAME}}_correct (reg : RegState) (size : Nat)
    (hv : validReg reg)
    (hne : ∃ x, inGamma reg x)
    (hsize : size = 1 ∨ size = 2 ∨ size = 4) :
    let r := {{LEAN_FUNC_NAME}} reg size
    -- Well-formedness: result is a valid register state
    validReg r ∧
    -- Soundness: every concrete result is in γ(r)
    (∀ x, inGamma reg x → inGamma r ({{LEAN_CONCRETE_OP_NAME}} x size)) ∧
