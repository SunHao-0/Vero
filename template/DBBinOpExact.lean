/-
  ...
  {{LEAN_OP_DESCRIPTION}} with an optional exact flag.
...
def {{LEAN_EXACT_PRE_NAME}} (x y : BitVec 64) : Prop :=
  {{LEAN_EXACT_PRE_BODY}}

/-- Combined flag precondition. -/
def {{LEAN_FLAGS_NAME}} (x y : BitVec 64) (exact : Bool) : Prop :=
  exact = true → {{LEAN_EXACT_PRE_NAME}} x y

/-- Compute optimal backward demanded bits for {{OPERATION_TITLE}}. -/
def {{LEAN_FUNC_NAME}} (demanded : BitVec 64) (a b : KnownBits) (exact : Bool)
    : BitVec 64 × BitVec 64 :=
-- === END: SPEC ===
...
theorem {{LEAN_FUNC_NAME}}_correct (demanded : BitVec 64) (a b : KnownBits) (exact : Bool)
    (ha : validKB a) (hb : validKB b) :
    let (dA, dB) := {{LEAN_FUNC_NAME}} demanded a b exact
    ...
    ((∀ (i : Fin 64), dA.getLsbD i.val = false →
        ∀ x y, inKBGamma a x → inKBGamma b y → y ≠ 0 → {{LEAN_FLAGS_NAME}} x y exact →
          inKBGamma a (flipBit x i.val) → {{LEAN_FLAGS_NAME}} (flipBit x i.val) y exact →
          ({{LEAN_CONCRETE_OP_NAME}} x y) &&& demanded = ({{LEAN_CONCRETE_OP_NAME}} (flipBit x i.val) y) &&& demanded) ∧
     (∀ (j : Fin 64), dB.getLsbD j.val = false →
        ∀ x y, inKBGamma a x → inKBGamma b y → y ≠ 0 → {{LEAN_FLAGS_NAME}} x y exact →
          inKBGamma b (flipBit y j.val) → (flipBit y j.val) ≠ 0 →
          {{LEAN_FLAGS_NAME}} x (flipBit y j.val) exact →
          ({{LEAN_CONCRETE_OP_NAME}} x y) &&& demanded = ({{LEAN_CONCRETE_OP_NAME}} x (flipBit y j.val)) &&& demanded)) ∧
    ...
    ((∀ (i : Fin 64), dA.getLsbD i.val = true →
        ∃ x₁ x₂ y, inKBGamma a x₁ ∧ inKBGamma a x₂ ∧ inKBGamma b y ∧ y ≠ 0 ∧
          {{LEAN_FLAGS_NAME}} x₁ y exact ∧ {{LEAN_FLAGS_NAME}} x₂ y exact ∧
          x₁ ^^^ x₂ = ((1 : BitVec 64) <<< i.val) ∧
          ({{LEAN_CONCRETE_OP_NAME}} x₁ y) &&& demanded ≠ ({{LEAN_CONCRETE_OP_NAME}} x₂ y) &&& demanded) ∧
     (∀ (j : Fin 64), dB.getLsbD j.val = true →
        ∃ x y₁ y₂, inKBGamma a x ∧ inKBGamma b y₁ ∧ inKBGamma b y₂ ∧
          y₁ ≠ 0 ∧ y₂ ≠ 0 ∧
          {{LEAN_FLAGS_NAME}} x y₁ exact ∧ {{LEAN_FLAGS_NAME}} x y₂ exact ∧
          y₁ ^^^ y₂ = ((1 : BitVec 64) <<< j.val) ∧
          ({{LEAN_CONCRETE_OP_NAME}} x y₁) &&& demanded ≠ ({{LEAN_CONCRETE_OP_NAME}} x y₂) &&& demanded)) := by
-- === END: SPEC_CORRECT ===
